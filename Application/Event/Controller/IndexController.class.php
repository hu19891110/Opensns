...
class IndexController extends Controller
{
    ...
    public function _initialize()
    {
        ...
        //----------tag部分的代码--------------------
        $rec_tag = D('freetags')->select();
        $this->assign('rec_tag', $rec_tag);
        //----------tag部分的代码--------------------
    ...
    public function index($page = 1, $type_id = 0, $norh = 'new')
    {
        ...
        $norh == 'hot' && $order = 'signCount desc';  //当 $norh == 'hot' 为真, $order = 'signCount desc' 是一个语句的时候,  $order = 'signCount desc' 就会运行.
    ...
    /**
     * 获取活动标签
     * 暂时用D('open_freetags'),后期改为Model
     * autor:liuchenwei
     */
    public function Tagview($type_id = 0, $page = 1, $tag_id = 0, $norh = 'new')//$tag_id = 0,暂时这里写定为norh为new,其他的以后再看
    {
        ...
        if ($tag_id == 0) {
            ...
            $content = D('tagview')->where($map)->order($order)->page($page, 10)->select();
        }
        $totalCount = D('tagview')->where($map)->count();
     ...
     * autor:xjw129xjt
     */
    public function myevent($page = 1, $type_id = 0, $lora = '',$tag_id = 0)
    {
     ...
     * autor:xjw129xjt
     */
    public function doPost($id = 0, $cover_id = 0, $title = '', $explain = '', $biaoqian ='',  $sTime = '', $eTime = '', $address = '', $limitCount = 0, $deadline = '', $type_id = 0)
    {
        if (!is_login()) {
            $this->error('请登陆后再投稿。');
        }
...
//同步到微博
     ...
     * autor:xjw129xjt
     */
    public function detail($id = 0,$type_id = 0)
    {
        //if($type_id!=0){
            $tree_d = D('EventType')->where(array('id' => 1))->select();
            $this->assign('tree_d', $tree_d);
            $rec_tag_d = D('Tagview')->where(array('status' => 1,'id'=>$id))->select();
            $this->assign('rec_tag_d', $rec_tag_d);
        //}
